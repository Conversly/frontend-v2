...
const VoiceAgentModal: React.FC<VoiceAgentModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden flex flex-col items-center p-8 relative">
        <button
          onClick={onClose}
        ...
        <button
          onClick={onClose}
          className="w-full py-4 rounded-xl font-semibold text-lg flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-xl active:scale-[0.98] bg-gray-200 text-gray-600"
        >
          Close
        </button>
      </div>
