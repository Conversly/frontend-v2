...
export const getWidgetConfig = async (
  ...
  const res = (await fetch(API.ENDPOINTS.DEPLOY.BASE_URL() + endpoint, {
    method: "GET",
    params: {
      chatbotId,
      playground,
...
export const getDomainAllowlist = async (
  chatbotId: string,
 ...
 */
export const rollbackDev = async (
  chatbotId: string,
...
export const getDeployStatus = async (
  ...
  const deployStatusField =
    res.data.deployStatusField ?? res.data.deployStatus ?? "SYNCED";
 ...
 */
export const getDeploymentDiff = async (
  chatbotId: string,
): Promise<DeploymentDiffResult> => {
