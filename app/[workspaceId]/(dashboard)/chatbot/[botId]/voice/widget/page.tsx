...
function VoiceWidgetLoader({ botId, workspaceId }: { botId: string, workspaceId: string }) {
    ...
    return <VoiceWidgetContent botId={botId} workspaceId={workspaceId} assistant={assistant} />;
}

function VoiceWidgetContent({ botId, workspaceId, assistant }: { botId: string, workspaceId: string, assistant: any }) {
    const router = useRouter();
